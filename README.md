...
概述
----
Hixy_MyMemories 是一个用于记录“记忆/纪念日/倒数日”的轻量 HarmonyOS 应用（ArkTS + Ark UI）。本报告以课程作业/实验报告的格式说明：项目目标、架构、关键实现点、我所做的修改（包含使用 @Prop/@Link/@StorageProp 等 ArkTS 装饰器的改进）、运行与测试步骤、设计权衡与后续建议。
...
------------------------------------
目标：修复“编辑页面无法正确接收 id 且提示“无法保存：未指定记录”的问题；同时把列表项左对齐并把列表项抽成可复用组件，使用 ArkTS 装饰器以符合更现代的写法。
