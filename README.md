...
下面列出了当前项目建议的待办项（可作为下一个迭代的工作清单），每项包含优先级建议、建议负责人与粗略工时估算，便于规划。
...
快速说明与建议流程：
- 先完成 1、2、3 三项（图标/主题/底栏）作为小而高价值的迭代，可显著提升体验与外观一致性。
- 并行推进第4项（测试）能降低回归风险，建议在实施重构（第6项）前先补充测试。
- 所有代码改动完成后，运行静态检查（get_errors）、格式化（项目 linter）并在 CI 中运行测试，确保流水线绿色。

可选扩展（长期）：
- 多端同步（云端 sync），需要后端或第三方同步服务；
- Widget / 桌面卡片支持，让倒数日更便捷可见；
- 用户帐号与数据导出/迁移（安全策略与隐私合规）。

如果你同意，我可以：
- 把上面的 TODO 条目同步到项目根目录下的 `TODO.md` 文件；
- 为优先级最高的 1-3 项创建对应的 issue 草稿（包含复现步骤/验收条件）；
- 开始按优先级逐条实现（先做图标与主题替换），并在每步后运行代码检查与单元测试。
